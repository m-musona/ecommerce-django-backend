...
def getProductsBySubCategory(request, category_id, sub_category_id):
    ...
    if category_id == "Electronics":
        electronics_sub_category = ElectronicsSubCategory.objects.get(
            id=sub_category_id
        )
        products = Product.objects.filter(
            electronics_sub_category=electronics_sub_category, category=category
        )
    elif category_id == "Fashion":
        fashion_sub_category = FashionSubCategory.objects.get(id=sub_category_id)
        products = Product.objects.filter(
            fashion_sub_category=fashion_sub_category, category=category
        )
    elif category_id == "Watches":
        watches_sub_category = WatchesSubCategory.objects.get(id=sub_category_id)
        products = Product.objects.filter(
            watches_sub_category=watches_sub_category, category=category
        )
    elif category_id == "Sports & Leisure":
        sports_and_leisure_sub_category = SportsAndLeisureSubCategory.objects.get(
            id=sub_category_id
        )
        products = Product.objects.filter(
        ...
        products = Product.objects.filter(
            appliances_sub_category=appliances_sub_category, category=category
        )
...
def searchForProducts(request):
    ...
    if query:
        ...
        return Response(
            "Please provide a valid search query.", status=status.HTTP_400_BAD_REQUEST
        )
